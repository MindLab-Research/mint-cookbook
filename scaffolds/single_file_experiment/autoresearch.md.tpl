...
- In the eval-first phase, wrapper runs are just fresh eval-only reruns with explicit log paths
- If training is later added, document in this file and in `README.md` which path is used for periodic train-time eval, which path is used for the final benchmark confirmation, and how to rerun a saved checkpoint cleanly
- When this experiment adopts the repo checkpoint contract, same-run resume should come from rerunning the same training command in the same run directory, `state_path` stays the resumable restore handle, `sampler_path` stays the later eval-only handle, and the wrapper recipe, this file, and `README.md` must stay aligned
