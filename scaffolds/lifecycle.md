...
5. Run `uv run train.py --dry-run` to validate data and prompt formatting without credentials.
6. Run `uv run train.py --eval-only` to establish the frozen benchmark baseline.
7. Decide whether the experiment stays eval-only or adds training.
8. If training is needed, choose exactly one extension: `scaffolds/profiles/sft.md` or `scaffolds/profiles/grpo.md`.

## Why This Lifecycle

If experiments start by choosing SFT vs GRPO, the repo drifts into incompatible shapes.
