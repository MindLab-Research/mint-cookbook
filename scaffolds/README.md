...
- Every experiment follows the same lifecycle: `--dry-run --eval-data <smoke_eval_path>` -> `--eval-only --eval-data <full_eval_path>` -> optional training.
- `uv run train.py --eval-only --eval-data <full_eval_path>` stays the benchmark confirmation entrypoint. Standard split-layout experiments should pass `data/eval/smoke.jsonl` for local validation and `data/eval/full.jsonl` for the frozen benchmark explicitly. Multi-dataset experiments may instead use documented dataset-family eval specs, but the path still must stay explicit. `autoresearch.sh` remains the canonical automation wrapper for the current practical line, and `autoresearch.md` should explain that wrapper instead of acting as loose scratch notes.
- New experiments should preserve their local data pipeline: either separate `data/eval/` and `data/train/` sides, or for multi-dataset work a documented dataset-family layout under `data/`. In both cases, keep raw snapshots when applicable, keep download or snapshot scripts plus build or adjustment scripts under `data/`, and materialize smoke before full when a cheap slice exists.
...
- `scaffolds/single_file_experiment/train.py.tpl` - canonical `train.py`; ships the eval path plus a generic SFT baseline distilled from `fingpt` / `lawbench` (shared CLI, explicit eval-data contract, generic `run_train`, automatic same-run resume helpers, and optional cadence-driven append streams), so supervised experiments can copy more directly and only replace task adapters or benchmark-specific training details
- `scaffolds/single_file_experiment/README.md.tpl` - experiment README; default top-level flow now follows `experiments/fingpt/README.md`: `Quickstart`, `Fast contract tests`, `Live smoke tests`, `Data`, `Current results`, `References`. Start `Current results` with `Status: \`placeholder\`` until a checked run exists, then switch it to `Status: \`checked\`` only when the reported run is actually checked; maintained experiments later keep that line aligned with `experiments/maintained.json`. Experiment-specific benchmark contract notes, run-mode notes, checkpoint semantics, and artifact pointers should usually live as subsections inside `Quickstart` or `Data` rather than as extra top-level sections. When stable result figures exist and are checked in, embed them directly in `Current results` instead of only mentioning the file path.
...
- `scaffolds/single_file_experiment/pyproject.toml.tpl` - dependencies; current scaffold pin follows the checked-in MinT compatibility chain: `mindlab-toolkit` + `tinker==0.15.0` + `transformers`. The package pin is still published under `tinker==0.15.0`, but scaffold-derived code should use `mint` runtime names by default.
- `scaffolds/single_file_experiment/env.tpl` - scaffold source for a local `.env` seed; do not treat it as a second checked-in `.env.example`
...
3. Run `uv run train.py --dry-run --eval-data <smoke_eval_path>`.
4. Run `uv run train.py --eval-only --eval-data <full_eval_path>`.
...
| `profiles/dpo.md` | Add pairwise preference optimization after eval is stable |
...
The promoted DPO profile is documentation-first: it standardizes the shared pairwise helper and resume shape without making DPO the canonical template default.
...
| Naming spec | `scaffolds/single_file_experiment/naming.md` | Stable infrastructure helper names; promoted SFT/DPO/RL helper names once the repo adopts the shared shape | Experiment-local grader names |
...
| `$new-experiment` skill | `skills/new-experiment/SKILL.md` | Bootstrap workflow plus scaffold-owned artifact/logging guidance for new experiments | Deep long-run upgrade playbooks |
| `$new-experiment-plus` skill | `skills/new-experiment-plus/SKILL.md`, `references/*.md` | Upgrade workflow, SFT/DPO/RL checklists, tinker-cookbook field mapping, throughput/resume guidance | Canonical baseline artifact naming |
...
- If you change the default README section flow or measured-run reporting convention for new experiments, update `README.md.tpl`, the relevant profiles (`eval.md`, `sft.md`, `dpo.md`, `grpo.md`), and any skills or reference docs that repeat that guidance in the same commit.
...
- Keep deep tinker-cookbook-specific field mapping in `skills/new-experiment-plus/references/logging.md` so this file stays repo-neutral.
...
- When stable result figures exist and are checked in, embed them directly in the results section instead of only pointing at the image file path.
...
1. keep train and eval as separate workflows under `data/train/` and `data/eval/`
   or, for multi-dataset experiments, keep a documented dataset-family layout under `data/`
2. preserve raw snapshots when local materialization is required
3. keep download or snapshot scripts under `data/`, next to the artifacts they feed
4. keep build or adjustment scripts under `data/`, next to the artifacts they materialize
...
Accepted variant: pairwise DPO eval may replace the generation-oriented `build_eval_messages(...)` / `grade_assistant_text(...)` path with pairwise scorers and a custom `run_eval(...)` that compares chosen vs rejected continuations on the same prompt. Keep the pairwise scoring rule explicit in the experiment and in `scaffolds/profiles/dpo.md`.
...
Benchmark-specific training policy, promoted DPO loops, richer logging, and other non-SFT variants remain explicit extension points.
...
- `experiments/dapo-aime` - GRPO runtime, checkpointing, and resume
...
| File | Purpose |
|------|---------|
| `train/metrics.jsonl` | Streamed per-step train diagnostics. SFT usually logs loss/lr/timing/throughput; DPO usually logs `dpo_loss`/accuracy/margin/lr/timing/throughput; RL usually logs reward/accuracy/format/datums/timing/throughput. |
| `train/batches.jsonl` (optional) | SFT prompt lineage or DPO pair lineage when reviewers need to see which examples reached a step. |
| `eval/periodic.jsonl` | SFT or DPO periodic eval history keyed by train step; may include bounded prediction samples. |
...
| `train/checkpoints.jsonl` | Checkpoint registry and restore source of truth. Rows should usually carry a human-readable logical `name`. SFT/DPO rows should carry `state_path`, `step`, and the next loop position (`epoch`, `batch`); RL rows should carry `state_path`, `completed_steps`, and `next_step`. `sampler_path` stays alongside those fields when later eval needs a durable sampler export, but the runtime save names behind `state_path` and `sampler_path` should stay distinct. |
...
- For DPO, keep the same phase-first naming and pairwise metric / preview shape as `experiments/chat-dpo`.
- For RL, keep the same phase-first naming and borrow record shape / loop structure from `experiments/dapo-aime`.
- The scaffold default keeps same-run resume directory-driven across SFT, DPO, and GRPO profiles whenever the current run directory actually has a resumable checkpoint row: rerun the same training command with the same run directory (exposed as `--log-path`). `train.py` auto-reads the latest resumable `state_path` from that run's `train/checkpoints.jsonl`, restores optimizer plus loop state from the matched row through a fresh LoRA training client and `load_state_with_optimizer(...)`, keeps the run-scoped JSONL, and appends `console.log`. No dedicated `--resume-from` flag is reserved for this default. SFT and DPO read loop state as `(step, epoch, batch)`; GRPO reads it as `(completed_steps, next_step)` from the same kind of checkpoint row.
...
- `experiments/dapo-aime` uses the same directory-driven training resume and `--load-checkpoint-path` behavior as the SFT/DPO experiments; `--eval-only` uses `--base-model` with a `sampler_path`. Prefer the checkpoint row dict from `get_last_resumable_checkpoint(...)` over experiment-local `ResumeLoopState` wrappers in new GRPO code.
- Do not gate automatic same-run resume on run-scoped append-only streams being exactly aligned to the last checkpoint row. SFT/DPO streams (`train/metrics.jsonl`, `train/batches.jsonl`, `eval/periodic.jsonl`) and RL streams (`train/metrics.jsonl`, `train/rollouts.jsonl`, `train/failures.jsonl`, `eval/metrics.jsonl`) may slide past the last resumable checkpoint without that being a resume failure.
...
- SFT/DPO rows should record completed optimizer `step` plus the next loop position as `epoch` and `batch`, matching the tinker-cookbook supervised resume shape.
...
- `--mint-timeout`
- `pyproject.toml.tpl` still pins `mindlab-toolkit` plus `tinker==0.15.0` because that package currently exposes the MinT-compatible runtime surface used by the maintained experiments and scaffold code

If you are migrating older code that already imports `tinker`, the narrow bridge is
`import mint as tinker`, then point endpoint and auth wiring at `MINT_BASE_URL`
and `MINT_API_KEY`. Keep that alias local to migration work; new scaffold-derived
code should still import `mint` directly.

Concrete experiments may intentionally diverge, but the runtime choice should stay self-consistent within that experiment's `README.md`, CLI flags, env keys, and backend compatibility section.
