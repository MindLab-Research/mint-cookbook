...
def load_live_env() -> dict[str, str]:
    ...
    env.setdefault("MINT_SHARED_CACHE_ROOT", DEFAULT_CACHE_ROOT)
    env.setdefault("UV_CACHE_DIR", f"{env['MINT_SHARED_CACHE_ROOT']}/uv")
    env.setdefault("XDG_CACHE_HOME", f"{env['MINT_SHARED_CACHE_ROOT']}/xdg")
    env.setdefault("PIP_CACHE_DIR", f"{env['MINT_SHARED_CACHE_ROOT']}/pip")
    env.setdefault("MPLCONFIGDIR", f"{env['MINT_SHARED_CACHE_ROOT']}/matplotlib")
    env.setdefault("WANDB_DIR", f"{env['MINT_SHARED_CACHE_ROOT']}/wandb")
    env.setdefault("HF_HOME", DEFAULT_HF_HOME)
    env.setdefault("HUGGINGFACE_HUB_CACHE", f"{env['HF_HOME']}/hub")
    env.setdefault("TRANSFORMERS_CACHE", f"{env['HF_HOME']}/hub")
    env.setdefault("HF_DATASETS_CACHE", f"{env['MINT_SHARED_CACHE_ROOT']}/hf-datasets")
...
class LiveDAPOAIME24FlowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.env = load_live_env()
        if not cls.env.get("MINT_API_KEY"):
            raise RuntimeError("missing live MINT_API_KEY for dapo-aime24 tests")

    def test_eval_only_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="dapo-aime24-eval-only-") as tmpdir:
            root = Path(tmpdir)
    ...
    def test_train_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="dapo-aime24-train-") as tmpdir:
            root = Path(tmpdir)
    ...
    def test_resume_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="dapo-aime24-resume-") as tmpdir:
            root = Path(tmpdir)
            train_path = root / "train.jsonl"
            eval_path = root / "eval.jsonl"
    ...
    def test_eval_only_from_saved_sampler_checkpoint(self):
        with tempfile.TemporaryDirectory(prefix="dapo-aime24-checkpoint-eval-") as tmpdir:
            root = Path(tmpdir)
            train_path = root / "train.jsonl"
            eval_path = root / "eval.jsonl"
