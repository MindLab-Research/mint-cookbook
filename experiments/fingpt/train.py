#!/usr/bin/env python3
"""Eval-first FinGPT benchmark scaffold with an M0 Fineval slice."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Append train/metrics.jsonl every N steps; first and last step are always kept.",
    )
    parser.add_argument(
        "--train-print-every",
        type=int,
        default=1,
        help="Print train-step progress every N steps; 0 disables step prints.",
    )
    parser.add_argument(
...
def build_supervised_batch_trace_record(
    *,
    ...
    """Per-step SFT batch lineage trace for ``train/batches.jsonl``."""
    ...
    for row_index, row in enumerate(batch_rows, start=1):
        prompt = str(row.get("instruction", "") or row.get("input", ""))
        assistant_text = str(row.get("output", ""))
...
def ensure_instruction_row(row: dict[str, Any], *, fallback_id: str) -> dict[str, Any]:
    instruction = str(pick_first(row, "instruction") or "").strip()
    input_text = str(pick_first(row, "input", "question", "prompt") or "").strip()
    output_text = str(pick_first(row, "output", "answer", "target", "label") or "").strip()
    if not instruction or not input_text or not output_text:
        raise RuntimeError(
            "Expected instruction/input/output fields in FinGPT row; "
            f"got keys {sorted(row.keys())}"
        )
    normalized = dict(row)
    normalized["id"] = extract_row_id(row, fallback=fallback_id)
...
def normalize_eval_rows(path: Path) -> list[dict[str, Any]]:
    ...
    return [ensure_instruction_row(row, fallback_id=f"eval-{idx:05d}") for idx, row in enumerate(rows, start=1)]


def normalize_train_rows(path: Path) -> list[dict[str, Any]]:
    rows, _ = load_records(path)
    return [ensure_instruction_row(row, fallback_id=f"train-{idx:05d}") for idx, row in enumerate(rows, start=1)]
...
def extract_choice_map(question: str) -> dict[str, str]:
    choices: dict[str, str] = {}
    for label, content in OPTION_BLOCK_RE.findall(question):
        normalized = content.strip()
        if normalized:
            choices[label.upper()] = normalized
    return choices


def canonical_choice_text(label: str, choices: dict[str, str]) -> str:
...
def build_eval_messages(row: dict[str, Any], task_type: str) -> list[dict[str, str]]:
    if task_type == "fineval":
        user_prompt = (
            f"{row['instruction']}\n\n"
            f"题目：\n{row['input']}\n\n"
            "请直接输出最终选项，优先使用与题目完全一致的格式，例如 `C. 1730万元`。"
            "如果你不能复述完整选项，至少输出单个选项字母。"
        )
        return [
            {
                "role": "system",
                "content": "你是金融考试助手。请先判断正确选项，再只输出最终答案，不要解释。",
            },
    ...
    user_prompt = (
        f"{row['instruction']}\n\n"
        f"Text:\n{row['input']}\n\n"
        "Only output the final label from the allowed set. Do not explain."
    )
    return [
        {
            "role": "system",
            "content": "You are a financial sentiment classifier. Output only the final label.",
        },
        {"role": "user", "content": user_prompt},
    ]


def instruction_label_space(instruction: str) -> list[str]:
    match = LABEL_BLOCK_RE.search(instruction)
    if not match:
        return []
    return [part.strip() for part in match.group(1).split("/") if part.strip()]


def extract_sentiment_label(text: str, allowed_labels: list[str]) -> str | None:
    normalized = normalize_text(text)
    if not normalized:
        return None
    candidates = sorted(
        ((label, normalize_text(label)) for label in allowed_labels),
    ...
    for _, prefix in (
        ...
        if normalized.startswith(prefix):
            ...
            break
    for label, label_norm in candidates:
        if normalized == label_norm or normalized.startswith(label_norm + " "):
            return label

    best_match: tuple[int, int, str] | None = None
    for label, label_norm in candidates:
        start = normalized.find(label_norm)
        if start < 0:
            continue
        boundary_before = start == 0 or not normalized[start - 1].isalpha()
        end = start + len(label_norm)
        boundary_after = end == len(normalized) or not normalized[end].isalpha()
        if not (boundary_before and boundary_after):
            continue
        candidate = (start, -len(label_norm), label)
        if best_match is None or candidate < best_match:
            best_match = candidate
    return best_match[2] if best_match is not None else None


def grade_assistant_text(
    assistant_text: str,
    row: dict[str, Any],
    task_type: str,
) -> tuple[bool, str, str | None]:
    prediction_text = assistant_text.strip()
    expected_text = row["output"]
    if task_type == "fineval":
        expected_label = extract_choice_label(expected_text)
        predicted_label = extract_choice_label(prediction_text)
        choices = extract_choice_map(row["input"])

        if expected_label is not None:
            canonical_prediction = (
                canonical_choice_text(predicted_label, choices)
                if predicted_label is not None
                else prediction_text
            )
            return predicted_label == expected_label, canonical_prediction, predicted_label

        is_correct = normalize_text(prediction_text) == normalize_text(expected_text)
        return is_correct, prediction_text, None

    allowed_labels = instruction_label_space(row["instruction"])
    expected_label = row["output"].strip()
    predicted_label = extract_sentiment_label(prediction_text, allowed_labels)
    canonical_prediction = predicted_label or prediction_text
    return predicted_label == expected_label, canonical_prediction, predicted_label


def compute_multiclass_f1(predictions: list[dict[str, Any]]) -> dict[str, float]:
    expected_labels = [str(p["expected"]) for p in predictions]
    predicted_labels = [str(p.get("predicted_label") or "") for p in predictions]
    labels = sorted(set(expected_labels) | {label for label in predicted_labels if label})
    total = len(predictions)
    if not labels or total == 0:
        return {"eval_micro_f1": 0.0, "eval_macro_f1": 0.0, "eval_weighted_f1": 0.0}
    ...
    for label in labels:
        ...
        support_sum += support
    return {
        "eval_micro_f1": sum(1 for p in predictions if p.get("correct")) / total,
        "eval_macro_f1": macro_sum / len(labels),
        "eval_weighted_f1": weighted_sum / support_sum if support_sum > 0 else 0.0,
    }
...
def compute_eval_metrics(
    ...
    if task_type == "sentiment":
        metrics.update(compute_multiclass_f1(predictions))
    return metrics
...
def run_dry_run(
    ...
    *,
    ...
    preview = build_eval_messages(eval_rows[0], args.task_type)
    print(
        "dry_run: "
        f"task_type={args.task_type} eval_set={eval_name} "
        f"eval_rows={len(eval_rows)} train_status={overlap['train_status']}"
    )
    ...
    *,
    ...
    async def eval_one(
        ...
        async with semaphore:
            messages = build_eval_messages(row, args.task_type)
            example_id = extract_row_id(row, fallback=f"example-{index}")
            eval_example = eval_example_artifact_row(
                row,
                messages=messages,
                example_id=example_id,
            )
            ...
            correct, prediction, predicted_label = grade_assistant_text(
                assistant_text, row, args.task_type
            )
            return eval_example, {
                "eval_uid": build_eval_uid(
                    eval_name="final",
    ...
    metrics = compute_eval_metrics(predictions, args.task_type)
...
async def run_train(
    ...
    tokenizer: Any,
    train_rows: list[dict[str, Any]],
    periodic_eval_name: str,
    periodic_eval_rows: list[dict[str, Any]],
    args: argparse.Namespace,
    log_path: Path,
) -> dict[str, float]:
    ...
    periodic_evals_path = log_path / "eval" / "periodic.jsonl"
    checkpoints_path = log_path / "train" / "checkpoints.jsonl"
    train_batches_path = log_path / "train" / "batches.jsonl"
    train_metrics_enabled = int(args.train_metrics_every) > 0
    ...
    # None on fresh runs and on --load-checkpoint-path runs.
    ...
    print(
        ...
        f"rank={args.rank} schedule={args.lr_schedule} "
        f"batch_prompt_rows_per_step={DEFAULT_BATCH_PROMPT_RECORD_LIMIT} "
        f"{'step_input_trace=train/batches.jsonl' if train_metrics_enabled else ''}"
    )
    if start_step > 0:
        print(
            f"Resume bookkeeping: start_step={start_step} start_epoch={start_epoch_idx} "
            f"start_batch={start_batch_idx} total_steps={total_steps}"
        )
    ...
    for epoch_idx in range(start_epoch_idx, num_epochs):
        # Make the shuffle order a pure function of (seed, epoch_idx) so that
        # resuming from (epoch, batch) replays the exact remaining batch order.
        ...
        for batch_idx in range(batch_start_idx, n_batches):
            ...
            learning_rate = base_lr * compute_lr_multiplier(
                str(args.lr_schedule), step, total_steps
            )
            batch_rows = shuffled[
                batch_idx * batch_size : (batch_idx + 1) * batch_size
            ]
            datums = [
                build_supervised_row_datum(tokenizer, row, args.task_type)
            ...
            if eval_every > 0 and step % eval_every == 0:
                ...
                _, periodic_preds, periodic_metrics = await run_eval(
                    sampler,
                    tokenizer,
                    periodic_eval_rows,
                    args,
                    dataset_name=periodic_eval_name,
                )
                step_eval_metrics = periodic_metrics
                periodic_scalars = scalar_metric_items(periodic_metrics)
                sample_preds = [
                    {k: v for k, v in p.items()
                     if k in ("id", "prediction", "correct", "expected", "assistant_text")}
                    for p in periodic_preds[:PERIODIC_EVAL_SAMPLE_COUNT]
                ]
                append_jsonl(
                    periodic_evals_path,
                    {"step": step, **periodic_scalars, "samples": sample_preds},
                )
                emit_metric_lines(periodic_scalars)
                print(
            ...
            if should_print_train_step(step, total_steps, int(args.train_print_every)):
                print(
                    ...
                    f"tok/s={tokens_per_second:.0f} step_time={step_time:.1f}s"
                )
        if max_steps > 0 and step >= max_steps:
            break
...
async def run_eval_bundle(
    sampler: Any,
    tokenizer: Any,
    eval_sets: list[tuple[str, Path, list[dict[str, Any]]]],
    args: argparse.Namespace,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    all_eval_examples: list[dict[str, Any]] = []
    all_predictions: list[dict[str, Any]] = []
    per_dataset: dict[str, dict[str, float]] = {}
    for dataset_name, _, eval_rows in eval_sets:
    ...
    aggregate_metrics = compute_eval_metrics(all_predictions, args.task_type)
    if len(per_dataset) > 1:
        for dataset_name, metrics in per_dataset.items():
            for key, value in scalar_metric_items(metrics).items():
                aggregate_metrics[f"{dataset_name}_{key.removeprefix('eval_')}"] = value
        metric_keys = [
            key
            for key in ("eval_accuracy", "eval_macro_f1", "eval_weighted_f1")
            if key in next(iter(per_dataset.values()))
        ]
        for key in metric_keys:
            aggregate_metrics[f"eval_mean_{key.removeprefix('eval_')}"] = sum(
                float(metrics[key]) for metrics in per_dataset.values()
            ) / len(per_dataset)
    return all_eval_examples, all_predictions, {
...
async def main_async() -> int:
    ...
    for dataset_name, eval_path in train_eval_sources:
        ...
        train_eval_sets.append((dataset_name, eval_path, eval_rows))
    ...
    combined_overlap = audit_overlap(
        [extract_row_id(r) for r in train_rows] if train_rows else None,
        [
            extract_row_id(r)
            for _, _, eval_rows in (eval_sets + train_eval_sets)
            for r in eval_rows
        ],
    )
    if combined_overlap["overlap_count"] > 0:
        raise RuntimeError(
            f"Train/eval overlap detected: {combined_overlap['overlap_preview']}"
        )

    eval_contract = [
        {"name": dataset_name, "path": str(eval_path), "rows": len(rows)}
        for dataset_name, eval_path, rows in eval_sets
    ]
    train_eval_contract = [
        {"name": dataset_name, "path": str(eval_path), "rows": len(rows)}
        for dataset_name, eval_path, rows in train_eval_sets
    ]
    ...
    if args.dry_run:
        ...
        return run_dry_run(
            args,
            eval_name=first_name,
            eval_rows=first_rows,
            overlap=combined_overlap,
            eval_path=first_path,
        )
    ...
    try:
        ...
        if args.eval_only:
            ...
            eval_examples, predictions, metrics_bundle = await run_eval_bundle(
                sampler, tokenizer, eval_sets, args
            )
            aggregate_metrics = metrics_bundle["aggregate"]
            write_outputs(
                run_dir,
                args=args,
                ...
                extra_payload={
                    "task_type": args.task_type,
                    "eval_datasets": eval_contract,
                    "train_data": str(train_path) if train_path else None,
                    "overlap": combined_overlap,
                },
            )
            emit_metric_lines(aggregate_metrics)
            write_run_metadata(
                run_dir,
                args,
                status="completed",
        ...
        train_metrics = await run_train(
            ...
            tokenizer,
            train_rows,
            periodic_eval_name=train_eval_sets[0][0] if train_eval_sets else "eval",
            periodic_eval_rows=train_eval_sets[0][2] if train_eval_sets else [],
            args=args,
            log_path=run_dir,
        )
        ...
        eval_metrics = metrics_bundle["aggregate"]
        write_outputs(
            run_dir,
            args=args,
            eval_examples=eval_examples,
            ...
            extra_payload={
                "task_type": args.task_type,
                "eval_datasets": eval_contract,
                "train_eval_datasets": train_eval_contract,
                "train_data": str(train_path) if train_path else None,
                "overlap": combined_overlap,
        ...
        emit_metric_lines({**train_metrics, **scalar_metric_items(eval_metrics)})
        write_run_metadata(
            run_dir,
            args,
            status="completed",
