...
- `train.py` expects explicit `--eval-data` / `--train-data` paths; use `data/smoke_*.jsonl` for dry-run harness validation before the official dataset is downloaded.
...
uv run train.py --eval-only --task-type fineval --eval-data fineval:data/fingpt-fineval/test.jsonl --base-model Qwen/Qwen3-4B-Instruct-2507
