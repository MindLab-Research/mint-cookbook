...
TRAIN_SMOKE_PATH = EXPERIMENT_DIR / "data" / "smoke_train.jsonl"
EVAL_SMOKE_PATH = EXPERIMENT_DIR / "data" / "smoke_eval.jsonl"
TRAIN_SMOKE_SPEC = "data/smoke_train.jsonl"
EVAL_SMOKE_SPEC = "smoke:data/smoke_eval.jsonl"
...
class LiveFinGPTFlowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.env = load_live_env()
        missing = [key for key in ("MINT_BASE_URL", "MINT_API_KEY") if not cls.env.get(key)]
        if missing:
            raise RuntimeError(f"missing live MinT env for FinGPT tests: {', '.join(missing)}")
        for path in (TRAIN_SMOKE_PATH, EVAL_SMOKE_PATH):
            if not path.is_file():
                raise RuntimeError(f"missing smoke data path: {path}")

    def test_eval_only_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="fingpt-eval-only-") as tmpdir:
            ...
            self.assertEqual(run_data["task_type"], "fineval")
            self.assertEqual(run_data["eval_datasets"][0]["name"], "smoke")
            self.assertEqual(run_data["eval_datasets"][0]["rows"], 2)
            self.assertEqual(len(predictions), 2)
            self.assertEqual(metrics["aggregate"]["eval_total"], 2.0)
            self.assertEqual(metrics["per_dataset"]["smoke"]["eval_total"], 2.0)

    def test_train_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="fingpt-train-") as tmpdir:
            ...
            self.assertEqual(run_data["status"], "completed")
            self.assertEqual(run_data["task_type"], "fineval")
            self.assertEqual(run_data["train_data"], TRAIN_SMOKE_SPEC)
            self.assertEqual(run_data["eval_datasets"][0]["name"], "smoke")
            self.assertEqual(run_data["train_metrics"]["train_steps"], 1.0)
            ...
            self.assertEqual(checkpoint_rows[-1]["name"], f"fingpt-train-{DEFAULT_MODEL_SLUG}-sft")
            self.assertIn("state_path", checkpoint_rows[-1])
            self.assertIn("sampler_path", checkpoint_rows[-1])
            self.assertEqual(eval_metrics["aggregate"]["eval_total"], 2.0)

    def test_resume_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="fingpt-resume-") as tmpdir:
            ...
            self.assertEqual(checkpoint_rows[0]["step"], 1)
            self.assertTrue(
                str(checkpoint_rows[-1].get("state_path") or "").strip(),
                "interrupted checkpoint row must carry a state_path so automatic "
                "same-run resume can pick it up",
            )

            # Automatic same-run resume: rerun the identical base_cmd in the same
            # --log-path. train.py reads the latest state_path from
            # train/checkpoints.jsonl via get_last_resumable_checkpoint(...) and
            # restores optimizer plus loop state. No --resume-from flag needed.
            ...
            eval_metrics = read_json(log_path / "eval" / "metrics.json")
            self.assertEqual(run_data["status"], "completed")
            # Same-run resume is directory-driven; --load-checkpoint-path must
            # stay empty (it is a fresh-run-only flag that is ignored once a
            # resumable state_path is already present in this --log-path).
    ...
    def test_sentiment_eval_only_live_smoke(self):
        with tempfile.TemporaryDirectory(prefix="fingpt-sentiment-eval-") as tmpdir:
            log_path = Path(tmpdir) / "sentiment-eval"
            eval_spec = "fpb:data/benchmarks/sentiment/fpb/test.jsonl"
            ...
            predictions = read_jsonl(log_path / "eval" / "predictions.jsonl")
            self.assertEqual(run_data["status"], "completed")
            self.assertEqual(run_data["task_type"], "sentiment")
            self.assertEqual(run_data["eval_datasets"][0]["name"], "fpb")
            self.assertEqual(run_data["eval_datasets"][0]["rows"], 2)
            self.assertEqual(len(predictions), 2)
            self.assertEqual(metrics["aggregate"]["eval_total"], 2.0)
            self.assertIn("eval_accuracy", metrics["aggregate"])
            self.assertIn("METRIC eval_accuracy=", completed.stdout)

    def test_eval_only_from_saved_sampler_checkpoint(self):
        with tempfile.TemporaryDirectory(prefix="fingpt-checkpoint-eval-") as tmpdir:
            root = Path(tmpdir)
