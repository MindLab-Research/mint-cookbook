#!/usr/bin/env python3
"""dapo-aime - direct GRPO on DAPO-Math-17k with AIME 2024/2025/2026 eval manifests."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--train-data", default=str(DEFAULT_TRAIN_DATA_PATH))
    parser.add_argument(
        "--eval-data",
        default="",
        help=(
            "Eval JSONL path or comma-separated eval specs. Each entry may be "
            "'name:path' or a plain path. Required for --dry-run, --eval-only, "
            "and final eval during training. Use data/eval/smoke.jsonl for "
            "local validation, data/eval/aime2024.jsonl for the frozen AIME "
            "2024 benchmark, or "
            "data/eval/aime2024.jsonl,data/eval/aime2025.jsonl,data/eval/aime2026.jsonl "
            "for combined multi-year runs."
        ),
    )
    parser.add_argument(
...
def parse_eval_data_arg(raw: str) -> list[tuple[str, Path]]:
    ...
    for entry in raw.split(","):
        ...
        if not name:
            raise RuntimeError(f"Invalid eval data entry without a dataset name: {entry!r}")
        path = Path(path_str)
        if not path.exists():
            raise RuntimeError(f"eval data not found at {path}")
        sources.append((name, path))
    if not sources:
        raise RuntimeError("--eval-data did not contain any usable entries")
    duplicate_names = sorted(
        {
            dataset_name
            for dataset_name, _ in sources
            if sum(1 for other_name, _ in sources if other_name == dataset_name) > 1
        }
    )
    if duplicate_names:
        raise RuntimeError(
            "Duplicate eval dataset names in --eval-data: "
...
def eval_metric_prefix(name: str) -> str:
    prefix = re.sub(r"[^a-z0-9]+", "_", name.strip().lower()).strip("_")
    if not prefix:
        raise RuntimeError(f"Eval dataset name must contain letters or digits: {name!r}")
    return prefix
...
def emit_named_eval_metric_lines(
    metrics_by_name: dict[str, dict[str, Any]],
    *,
    skip_name: str | None = None,
) -> None:
    for eval_name, metrics in metrics_by_name.items():
        if skip_name is not None and eval_name == skip_name:
            continue
        prefix = eval_metric_prefix(eval_name)
        emit_metric_lines(
            {
                f"{prefix}_{metric_name}": metric_value
                for metric_name, metric_value in scalar_metric_items(metrics).items()
            }
        )
...
def build_state_save_name(base_model: str, log_path: Path) -> str:
    ...
    return f"{EXPERIMENT_DIR.name}-{output_slug}-{model_slug}-grpo"
...
def parse_step_from_checkpoint_path(state_path: str) -> int | None:
    ...
    step_match = re.search(r"(?:^|-)step-(\d+)(?:-(?:state|sampler))?$", checkpoint_name)
...
def resume_loop_state_from_explicit_state_path(
    ...
    for checkpoints_path in candidate_checkpoint_record_paths(output_dir):
        for record in reversed(load_existing_jsonl(checkpoints_path)):
...
def build_periodic_eval_rows(
    eval_sets: list[tuple[str, Path, list[dict[str, Any]]]],
    *,
    seed: int,
) -> tuple[str, list[dict[str, Any]], list[dict[str, Any]]]:
    if not eval_sets:
        raise RuntimeError("At least one eval dataset is required")
    if len(eval_sets) == 1:
        dataset_name, eval_path, rows = eval_sets[0]
        return (
            dataset_name,
            rows,
            [
                {
                    ...
                    "source_eval_data": str(eval_path),
    ...
    for dataset_name, eval_path, rows in eval_sets:
        selected_rows = max(len(rows) // 3, 1)
        shuffled_rows = list(rows)
        random.Random(f"{seed}:{dataset_name}:{eval_path}").shuffle(shuffled_rows)
        chosen_rows = sorted(
            shuffled_rows[:selected_rows],
            key=lambda row: int(row.get("_source_index") or 0),
        )
        mixed_rows.extend(chosen_rows)
        selection_summary.append(
            {
                ...
                "total_rows": len(rows),
                "selection": "stable_random_third",
            }
        )
    random.Random(f"{seed}:periodic_eval_mix").shuffle(mixed_rows)
    return "train_mix", mixed_rows, selection_summary
    ...
    with predictions_path.open(
        ...
        def process_eval_result(request_index: int, result: Any) -> None:
            ...
            record = {
                ...
                "source_index": int(row.get("_source_index") or index),
...
def run_dry_run(
    ...
    *,
    ...
    periodic_eval_summary: list[dict[str, Any]],
    args: argparse.Namespace,
) -> None:
    train_status = "missing"
    train_rows = 0
    ...
    eval_source_summary = ",".join(
        f"{dataset_name}:{len(rows)}" for dataset_name, _path, rows in eval_sets
    )
    print(
        "dry_run: "
        f"train_data={os.path.relpath(train_data_path, Path.cwd())} "
        f"train_status={train_status} train_rows={train_rows} "
        f"final_eval_sources={eval_source_summary} periodic_eval_name={periodic_eval_name} "
        f"periodic_eval_rows={len(periodic_eval_rows)} grpo_steps={args.grpo_steps} "
        f"group_size={args.group_size}"
    )
    if periodic_eval_summary:
        print(
            "dry_run: periodic_eval_selection="
            + json.dumps(periodic_eval_summary, ensure_ascii=False, sort_keys=True)
        )
    ...
    print(
        preview_messages(periodic_eval_rows[0]["messages"])
...
def reset_eval_output_artifacts(run_dir: Path) -> None:
    ...
    for child in eval_dir.iterdir():
        if child.name in {"metrics.jsonl", "steps"}:
            continue
        if child.is_dir():
            remove_path(child)
    ...
    *,
    ...
    if eval_dir.is_dir():
        for dataset_dir in sorted(eval_dir.iterdir()):
            if not dataset_dir.is_dir() or dataset_dir.name == "steps":
                continue
            dataset_prefix = eval_metric_prefix(dataset_dir.name)
            for artifact_name, artifact_path in (
                ("examples", dataset_dir / "examples.jsonl"),
                ("predictions", dataset_dir / "predictions.jsonl"),
                ("metrics", dataset_dir / "metrics.json"),
                ("failures_log", dataset_dir / "failures.log"),
                ("failures", dataset_dir / "failures.jsonl"),
            ):
                if artifact_path.is_file() and artifact_path.stat().st_size > 0:
                    eval_artifacts[f"{dataset_prefix}_{artifact_name}"] = str(
                        artifact_path
                    )
...
def write_outputs(
    ...
    *,
    ...
    if predictions is not None:
        ...
        write_jsonl(predictions_path, prediction_artifact_rows(predictions))
...
async def main_async() -> int:
    ...
    for dataset_name, eval_path in eval_sources:
        eval_sets.append(
            (
                ...
                eval_path,
                annotate_eval_rows(
                    normalize_eval_rows(eval_path),
                    dataset_name=dataset_name,
                    eval_path=eval_path,
                ),
            )
        )
    primary_eval_name, primary_eval_path, _primary_eval_rows = eval_sets[0]
    (
        periodic_eval_name,
        periodic_eval_rows,
        periodic_eval_summary,
    ) = build_periodic_eval_rows(eval_sets, seed=args.seed)
    ...
    if args.dry_run:
        run_dry_run(
            train_path,
            eval_sets,
            periodic_eval_name=periodic_eval_name,
            periodic_eval_rows=periodic_eval_rows,
    ...
    try:
        ...
        periodic_eval_history: list[dict[str, Any]] = []
        cached_periodic_final_eval: dict[str, float] | None = None
        final_sampler_path: str | None = None
        if not args.eval_only:
            (
                rl_metrics,
                periodic_eval_history,
                cached_periodic_final_eval,
                final_sampler_path,
        ...
        if args.save_state_name:
            ...
            final_checkpoint_name = args.save_state_name
            final_state_name, final_sampler_name = checkpoint_save_names(
                final_checkpoint_name
            )
            state_path = await save_training_state(
                training_client, final_state_name
            )
            final_sampler_path = await save_sampler_checkpoint(
                training_client, final_sampler_name
            )
        ...
        if cached_periodic_final_eval is not None:
            print(
                ">> periodic_eval_final_step_cached=1 final_eval_rerun=1 "
                "reason=formal_final_artifacts",
                flush=True,
            )

        final_eval_results: list[tuple[str, Path, dict[str, float]]] = []
        if eval_sampler is None:
            if training_client is None:
                raise RuntimeError("No evaluation sampler available")
            eval_sampler = await save_weights_for_sampling(training_client)
        use_named_eval_dirs = len(eval_sets) > 1
        for eval_name, eval_path, eval_rows in eval_sets:
            eval_output_dir = (
                run_dir / "eval" / eval_name
                if use_named_eval_dirs
                else run_dir / "eval"
            )
            eval_metrics = await evaluate_with_sampler(
                ...
                eval_output_dir,
                log_label=(
                    f"eval_{eval_metric_prefix(eval_name)}"
                    if use_named_eval_dirs
                    else "eval"
                ),
                verbose_item_logs=True,
            )
            final_eval_results.append((eval_name, eval_path, eval_metrics))
        primary_eval_name, primary_eval_path, primary_eval_metrics = final_eval_results[
            0
        ]
        eval_metrics_by_name = {
            eval_name: eval_metrics
            for eval_name, _eval_path, eval_metrics in final_eval_results
        }
        eval_data_paths = {
            eval_name: str(eval_path)
            for eval_name, eval_path, _eval_metrics in final_eval_results
        }
        summary_metrics: dict[str, Any] = primary_eval_metrics
        if len(final_eval_results) > 1:
            summary_metrics = {
                "primary_eval_name": primary_eval_name,
                "primary_eval_path": str(primary_eval_path),
                "eval_metrics": primary_eval_metrics,
                "eval_metrics_by_name": eval_metrics_by_name,
                "eval_data_paths": eval_data_paths,
            }
        state_path_file = run_dir / "train" / "state_path.txt"
        if state_path:
        ...
        write_outputs(
            ...
            extra_payload={
                ...
                "eval_data_paths": eval_data_paths,
                "periodic_eval_name": periodic_eval_name,
                "periodic_eval_rows": len(periodic_eval_rows),
                "periodic_eval_selection": periodic_eval_summary,
                "state_path": state_path,
                "periodic_eval_history": periodic_eval_history,
            },
            include_existing_artifacts=metadata_includes_append_streams,
        )
        emit_metric_lines(primary_eval_metrics)
        emit_named_eval_metric_lines(
            eval_metrics_by_name,
            skip_name=primary_eval_name,
        )
