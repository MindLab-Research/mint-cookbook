#!/usr/bin/env python3
"""Eval-first LawBench experiment with a cookbook execution baseline and a minimal SFT path."""
...
OFFICIAL_TASK_SCORER_SPECS = {
    "1-1": ("evaluation_functions.ftcs", "compute_ftcs"),
    "1-2": ("evaluation_functions.jec_kd", "compute_jec_kd"),
    "2-1": ("evaluation_functions.wsjd", "compute_wsjd"),
    "2-2": ("evaluation_functions.jdzy", "compute_jdzy"),
    "2-3": ("evaluation_functions.wbfl", "compute_wbfl"),
    "2-4": ("evaluation_functions.zxfl", "compute_zxfl"),
    "2-5": ("evaluation_functions.ydlj", "compute_ydlj"),
    "2-6": ("evaluation_functions.xxcq", "compute_xxcq"),
    "2-7": ("evaluation_functions.yqzy", "compute_yqzy"),
    "2-8": ("evaluation_functions.lblj", "compute_lblj"),
    "2-9": ("evaluation_functions.sjjc", "compute_sjjc"),
    "2-10": ("evaluation_functions.sjjc", "compute_cfcy"),
    "3-1": ("evaluation_functions.ljp_article", "compute_ljp_article"),
    "3-2": ("evaluation_functions.cjft", "compute_cjft"),
    "3-3": ("evaluation_functions.ljp_accusation", "compute_ljp_accusation"),
    "3-4": ("evaluation_functions.ljp_imprison", "compute_ljp_imprison"),
    "3-5": ("evaluation_functions.ljp_imprison", "compute_ljp_imprison"),
    "3-6": ("evaluation_functions.jec_ac", "compute_jec_ac"),
    "3-7": ("evaluation_functions.jetq", "compute_jetq"),
    "3-8": ("evaluation_functions.flzx", "compute_flzx"),
}
...
def apply_offline_env_defaults() -> None:
    # lawbench-specific SPECIALIZATION: ensures HuggingFace offline mode on the
    # shared cluster. Do NOT backflow this helper into the scaffold template.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train-eval-data",
        default="",
        help="Eval data used during periodic training evals. Defaults to --eval-data if omitted.",
    )
    ...
    parser.add_argument(
        "--eval-max-tokens",
        type=int,
        default=int(os.environ.get("LAWBENCH_EVAL_MAX_TOKENS", "1024")),
    )
    ...
    parser.add_argument(
        "--rank",
        type=int,
        default=int(os.environ.get("LAWBENCH_LORA_RANK", "16")),
    )
    parser.add_argument(
        "--learning-rate",
        type=float,
        default=float(os.environ.get("LAWBENCH_LEARNING_RATE", "1e-4")),
    )
    parser.add_argument("--num-epochs", type=int, default=1, help="Number of training epochs.")
    parser.add_argument("--max-steps", type=int, default=0, help="Cap total steps; 0 = train full epochs.")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=int(os.environ.get("LAWBENCH_BATCH_SIZE", "8")),
    )
...
def temporary_sys_path(paths: list[Path]) -> Any:
    inserted: list[str] = []
    for path in reversed(paths):
        path_s = str(path)
        if path_s not in sys.path:
            sys.path.insert(0, path_s)
            inserted.append(path_s)
    try:
        yield
    finally:
        for path_s in inserted:
            while path_s in sys.path:
                sys.path.remove(path_s)


@contextlib.contextmanager
def temporary_cwd(path: Path) -> Any:
    previous = Path.cwd()
    os.chdir(path)
    try:
        yield
...
def build_assistant_message(tokenizer: Any, sample_result: Any) -> dict[str, str]:
    assistant_text = tokenizer.decode(extract_completion_tokens(sample_result)).strip()
...
def extract_assistant_text(message: dict[str, Any]) -> str:
    ...
    if not isinstance(content, str):
        raise RuntimeError("Assistant message must contain string content")
    return content
...
async def sample_assistant_message(
    ...
    if callable(sample_fn):
        ...
        result = await asyncio.to_thread(lambda: resolve_api_result(sample_fn(**kwargs)))
    return build_assistant_message(tokenizer, result)


# ===== SFT / training helpers =====
...
def compute_total_train_steps(
    ...
    *,
    ...
    if n_batches == 0:
        raise RuntimeError(f"Not enough train rows ({train_row_count}) for batch_size={batch_size}")
...
def compute_mean_nll(fwd_bwd_result: Any, datums: list[Any]) -> tuple[float, float]:
    ...
    Accepts either an object exposing ``loss_fn_outputs`` or a dict payload with
    the same key so the helper works across mint runtime response shapes.
    Returns ``(nan, 0.0)`` when the batch has zero total weight.
    """
...
def validate_resume_contract(
    ...
    *,
    ...
    for key, current_value in {
        ...
        "train_eval_data": str(getattr(args, "train_eval_data", "") or ""),
        "seed": args.seed,
        "rank": args.rank,
...
def build_supervised_batch_trace_record(
    *,
    ...
    level_counts = {level: 0 for level in ("memory", "understanding", "application")}
    task_names: list[str] = []
    ...
    for row_index, row in enumerate(batch_rows, start=1):
        ...
        prompt_tokens = count_text_tokens(tokenizer, prompt, add_special_tokens=True)
        assistant_text_tokens = count_text_tokens(tokenizer, assistant_text, add_special_tokens=False)
            ...
            examples.append(
                {
                    ...
                    "assistant_text_loss_tokens": assistant_text_loss_tokens,
    ...
    return {
        ...
        "cognitive_level_counts": {level: count for level, count in level_counts.items() if count > 0},
...
def ensure_unique_example_ids(rows: list[dict[str, Any]], path: Path) -> None:
    seen: set[str] = set()
    for index, row in enumerate(rows, start=1):
        eid = extract_row_id(row)
        if not eid:
            raise RuntimeError(f"{path}:{index} is missing example_id")
        if eid in seen:
            raise RuntimeError(f"Duplicate example_id `{eid}` in {path}")
        seen.add(eid)


def validate_train_rows(rows: list[dict[str, Any]], path: Path) -> None:
    if not rows:
        raise RuntimeError(f"Train data is empty: {path}")
    ensure_unique_example_ids(rows, path)
    for index, row in enumerate(rows, start=1):
        for key in ("task_name", "cognitive_level", "prompt", "assistant_text"):
            value = row.get(key)
            if not isinstance(value, str) or not value.strip():
                raise RuntimeError(f"{path}:{index} missing non-empty string field `{key}`")
        level = str(row["cognitive_level"]).strip().lower()
        if level not in SUPPORTED_LEVELS:
            raise RuntimeError(f"{path}:{index} uses unsupported cognitive_level `{level}`")


def validate_eval_rows(rows: list[dict[str, Any]], path: Path) -> None:
    if not rows:
        raise RuntimeError(f"Eval data is empty: {path}")
    ensure_unique_example_ids(rows, path)
    for index, row in enumerate(rows, start=1):
        for key in ("task_name", "cognitive_level", "prompt"):
            value = row.get(key)
            if not isinstance(value, str) or not value.strip():
                raise RuntimeError(f"{path}:{index} missing non-empty string field `{key}`")
        expected = row.get("expected")
        if not isinstance(expected, str):
            raise RuntimeError(f"{path}:{index} missing string field `expected`")
        level = str(row["cognitive_level"]).strip().lower()
        if level not in SUPPORTED_LEVELS:
            raise RuntimeError(f"{path}:{index} uses unsupported cognitive_level `{level}`")
        task_id = str(row.get("task_id") or "").strip()
        if not task_id:
            raise RuntimeError(f"{path}:{index} requires non-empty task_id for official scoring")
        if task_id not in OFFICIAL_TASK_SCORER_SPECS:
            raise RuntimeError(f"{path}:{index} uses unsupported official task_id `{task_id}`")


def render_eval_prompt(row: dict[str, Any]) -> str:
    return str(row["prompt"])


def metric_levels(rows: list[dict[str, Any]]) -> dict[str, int]:
    counts = {level: 0 for level in sorted(SUPPORTED_LEVELS)}
    for row in rows:
        counts[str(row["cognitive_level"]).strip().lower()] += 1
    return counts
...
def format_prediction_for_artifact(assistant_text: str) -> str:
    """Human-readable prediction text for ``eval/predictions.jsonl`` (scores still use raw assistant text)."""
    collapsed = strip_special_tokens(assistant_text)
...
def normalize_eval_rows(path: Path) -> list[dict[str, Any]]:
    """Load, validate, and return eval rows."""
    rows = load_jsonl(path)
    validate_eval_rows(rows, path)
    return rows


def normalize_train_rows(path: Path) -> list[dict[str, Any]]:
    """Load, validate, and return train rows."""
    rows = load_jsonl(path)
    validate_train_rows(rows, path)
    return rows


def build_eval_messages(row: dict[str, Any]) -> list[dict[str, str]]:
    """Build chat messages from an eval row."""
    prompt = render_eval_prompt(row)
    return [{"role": "user", "content": prompt}]


def build_official_scorer_record(row: dict[str, Any], prediction_text: str) -> dict[str, str]:
    return {
        "origin_prompt": str(row.get("prompt") or ""),
        "prediction": prediction_text,
        "refr": str(row.get("expected") or ""),
    }


def grade_assistant_text(assistant_text: str, row: dict[str, Any]) -> tuple[bool, str]:
    raise RuntimeError(
        "LawBench grading is official-scorer-only at the task level; use run_eval() or compute_eval_metrics()."
    )


def build_official_scorer_task_records(
    eval_rows: list[dict[str, Any]],
    predictions: list[dict[str, Any]],
) -> dict[str, list[dict[str, str]]]:
    if len(eval_rows) != len(predictions):
        raise RuntimeError("Official scorer requires eval_rows and predictions to have matching lengths")
    grouped: dict[str, list[dict[str, str]]] = {}
    for row, prediction in zip(eval_rows, predictions):
        task_id = str(row.get("task_id") or prediction.get("task_id") or "").strip()
        if not task_id:
            raise RuntimeError("Official scorer requires eval rows with task_id for every example")
        raw_assistant_text = str(
            prediction.get("_raw_assistant_text")
            or prediction.get("assistant_text")
            or prediction.get("prediction")
            or ""
        )
        grouped.setdefault(task_id, []).append(build_official_scorer_record(row, raw_assistant_text))
    return grouped


def task_id_sort_key(task_id: str) -> tuple[int, int]:
    left, right = str(task_id).split("-", 1)
    return int(left), int(right)


def group_eval_rows_by_task(
    eval_rows: list[dict[str, Any]],
    ...
    for index, row in enumerate(eval_rows, start=1):
        ...
        if not task_id:
            raise RuntimeError("Official scorer requires eval rows with task_id for every example")
        grouped.setdefault(task_id, []).append((index, row))
    return grouped


def build_task_metric_record(
    task_id: str,
    rows: list[dict[str, Any]],
    result: dict[str, float],
) -> dict[str, Any]:
    if not rows:
        raise RuntimeError(f"Cannot build task metric record for empty task `{task_id}`")
    exemplar = rows[0]
    return {
        "task_id": task_id,
        "task_name": str(exemplar.get("task_name") or ""),
        "cognitive_level": str(exemplar.get("cognitive_level") or "").strip().lower(),
...
def load_official_task_scorer(task_id: str) -> Any:
    module_name, function_name = OFFICIAL_TASK_SCORER_SPECS.get(task_id, (None, None))
    if not module_name or not function_name:
        raise RuntimeError(f"No official scorer is registered for task_id `{task_id}`")
    if not OFFICIAL_SCORER_DIR.is_dir():
        raise RuntimeError(f"Official scorer directory is missing: {OFFICIAL_SCORER_DIR}")
    try:
        with temporary_sys_path([OFFICIAL_SCORER_DIR, OFFICIAL_SCORER_DIR / "utils"]):
            module = importlib.import_module(module_name)
    except ModuleNotFoundError as exc:
        missing = exc.name or "unknown"
        raise RuntimeError(
            "LawBench official scorer dependency missing: "
            f"{missing}. Install the vendored scorer runtime dependencies first."
        ) from exc
    scorer = getattr(module, function_name, None)
    if not callable(scorer):
        raise RuntimeError(f"Official scorer function `{function_name}` is missing in `{module_name}`")
    return scorer


def check_official_scorer_wiring(eval_rows: list[dict[str, Any]]) -> list[str]:
    """Import every scorer needed by the selected eval rows."""
    task_ids = sorted({str(row.get("task_id") or "").strip() for row in eval_rows}, key=task_id_sort_key)
    for task_id in task_ids:
        if not task_id:
            raise RuntimeError("Official scorer wiring check requires non-empty task_id values")
        load_official_task_scorer(task_id)
    return task_ids


def run_official_task_scorer(task_id: str, records: list[dict[str, str]]) -> dict[str, float]:
    scorer = load_official_task_scorer(task_id)
    with temporary_cwd(OFFICIAL_SCORER_DIR):
        result = scorer(records)
    if not isinstance(result, dict) or "score" not in result:
        raise RuntimeError(f"Official scorer for task {task_id} returned unexpected payload: {type(result).__name__}")
    return {
        "score": float(result.get("score") or 0.0),
        # UPSTREAM: LawBench official scorers emit a typo key `anstention_rate`
        # in several tasks; the canonical `abstention_rate` is kept as first
        # lookup. Do not fix the typo in the vendored scorers — it is part of
        # the upstream third_party/lawbench_official source tree.
        "abstention_rate": float(result.get("abstention_rate", result.get("anstention_rate", 0.0)) or 0.0),
    }


def aggregate_official_task_metrics(
    *,
    task_scores: dict[str, float],
    task_abstention_rates: dict[str, float],
    task_levels: dict[str, str],
    task_sizes: dict[str, int],
) -> dict[str, float]:
    if not task_scores:
        return {
            "eval_lawbench_avg": 0.0,
            "eval_mem_avg": 0.0,
            "eval_understanding_avg": 0.0,
            "eval_application_avg": 0.0,
            "eval_abstention_rate": 0.0,
        }
    level_scores = {level: [] for level in SUPPORTED_LEVELS}
    for task_id, score in task_scores.items():
        level = str(task_levels.get(task_id) or "").strip().lower()
        if level in level_scores:
            level_scores[level].append(score)
    total_examples = sum(task_sizes.get(task_id, 0) for task_id in task_scores)
    weighted_abstention = 0.0
    if total_examples > 0:
        weighted_abstention = sum(
            task_abstention_rates.get(task_id, 0.0) * task_sizes.get(task_id, 0)
            for task_id in task_scores
        ) / total_examples
    overall = sum(task_scores.values()) / len(task_scores)
    return {
        "eval_lawbench_avg": overall,
        "eval_mem_avg": sum(level_scores["memory"]) / len(level_scores["memory"]) if level_scores["memory"] else 0.0,
        "eval_understanding_avg": sum(level_scores["understanding"]) / len(level_scores["understanding"]) if level_scores["understanding"] else 0.0,
        "eval_application_avg": sum(level_scores["application"]) / len(level_scores["application"]) if level_scores["application"] else 0.0,
        "eval_abstention_rate": weighted_abstention,
    }


def finalize_eval_metrics_from_task_records(task_records: list[dict[str, Any]]) -> dict[str, Any]:
    task_scores = {str(record["task_id"]): float(record["score"]) for record in task_records}
    task_abstention_rates = {
        str(record["task_id"]): float(record.get("abstention_rate", 0.0))
        for record in task_records
    }
    task_levels = {
        str(record["task_id"]): str(record.get("cognitive_level") or "").strip().lower()
        for record in task_records
    }
    task_sizes = {str(record["task_id"]): int(record.get("num_examples") or 0) for record in task_records}
    aggregated = aggregate_official_task_metrics(
        task_scores=task_scores,
        task_abstention_rates=task_abstention_rates,
        task_levels=task_levels,
        task_sizes=task_sizes,
    )
    aggregated["eval_task_metrics"] = {
        str(record["task_id"]): {
            "task_name": str(record.get("task_name") or ""),
            "cognitive_level": str(record.get("cognitive_level") or ""),
            "score": float(record["score"]),
            "abstention_rate": float(record.get("abstention_rate", 0.0)),
            "num_examples": int(record.get("num_examples") or 0),
        }
        for record in sorted(task_records, key=lambda item: task_id_sort_key(str(item["task_id"])))
    }
    return aggregated
...
def prediction_artifact_rows(predictions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {key: value for key, value in prediction.items() if not str(key).startswith("_")}
...
def persist_eval_progress(
    ...
    *,
    ...
    metrics = finalize_eval_metrics_from_task_records(task_records)
    write_outputs(
        log_path,
        args=args,
        ...
        extra_payload={
            ...
            "eval_data": eval_data,
            "overlap": overlap,
            "run_duration_seconds": time.time() - started_at,
            "eval_progress": {
                "completed_task_count": len(task_records),
                "completed_task_ids": [str(record["task_id"]) for record in task_records],
                "completed_example_count": len(predictions),
                "total_task_count": total_task_count,
                "last_completed_task_id": completed_task_id,
            },
        },
    )
    return metrics
...
def compute_eval_metrics(
    ...
    *,
    ...
    """Aggregate prediction list with the vendored official LawBench scorer."""
    grouped = build_official_scorer_task_records(eval_rows, predictions)
    grouped_rows = group_eval_rows_by_task(eval_rows)
    task_records: list[dict[str, Any]] = []
    for task_id in sorted(grouped, key=task_id_sort_key):
        task_rows = [row for _, row in grouped_rows[task_id]]
        task_records.append(
            build_task_metric_record(task_id, task_rows, run_official_task_scorer(task_id, grouped[task_id]))
        )
    return finalize_eval_metrics_from_task_records(task_records)
...
def run_dry_run(
    ...
    *,
    ...
    levels = metric_levels(eval_rows)
    preview = build_eval_messages(eval_rows[0])
    scorer_task_ids = check_official_scorer_wiring(eval_rows)
    print(f"dry_run: eval_rows={len(eval_rows)} train_status={overlap['train_status']}")
    print(
        "dry_run: eval_level_counts="
        + ", ".join(f"{level}:{levels[level]}" for level in ("memory", "understanding", "application"))
    )
    print(f"dry_run: overlap_count={overlap['overlap_count']}")
    print("dry_run: selected_eval_setting=zero-shot")
    print(
        "dry_run: official_scorer_wiring="
        f"ok task_count={len(scorer_task_ids)} task_ids={','.join(scorer_task_ids)}"
    )
    print(
        "dry_run: sft_logging="
        f"train_metrics_every={int(args.train_metrics_every)} "
        f"train_print_every={int(args.train_print_every)} "
        f"batch_prompt_rows_per_step={DEFAULT_BATCH_PROMPT_RECORD_LIMIT} "
        "step_input_trace=train/batches.jsonl"
    )
    print("dry_run: prompt_preview_start")
    print(json.dumps(preview, indent=2, ensure_ascii=False))
    print("dry_run: prompt_preview_end")
    print(f"dry_run: expected_answer={eval_rows[0]['expected']}")
    return 0
...
async def run_eval(
    ...
    args: argparse.Namespace,
    *,
    on_task_complete: Callable[[list[dict[str, Any]], list[dict[str, Any]], str], Awaitable[None] | None] | None = None,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    """Evaluate LawBench task_id-by-task_id, batching only within each task."""
    batch_size = max(int(args.max_concurrent_requests), 1)
    grouped_rows = group_eval_rows_by_task(eval_rows)
    eval_examples = [
    ...
    async def eval_one(index: int, row: dict[str, Any]) -> dict[str, Any]:
        messages = build_eval_messages(row)
        assistant_message = await sample_assistant_message(sampler, tokenizer, messages, args)
        assistant_text = extract_assistant_text(assistant_message)
        return {
            "id": extract_row_id(row, fallback=f"example-{index}"),
            ...
            "prediction": format_prediction_for_artifact(assistant_text),
            "assistant_text": assistant_text,
            "_raw_assistant_text": assistant_text,
        }

    predictions: list[dict[str, Any]] = []
    task_records: list[dict[str, Any]] = []
    for task_id in sorted(grouped_rows, key=task_id_sort_key):
        task_entries = grouped_rows[task_id]
        task_predictions: list[dict[str, Any]] = []
        for start in range(0, len(task_entries), batch_size):
            batch_entries = task_entries[start : start + batch_size]
            task_predictions.extend(
                await asyncio.gather(*(eval_one(index, row) for index, row in batch_entries))
            )
        predictions.extend(task_predictions)
        task_rows = [row for _, row in task_entries]
        task_records.append(
            build_task_metric_record(
                task_id,
                task_rows,
                run_official_task_scorer(
                    task_id,
                    [
                        build_official_scorer_record(row, str(prediction["_raw_assistant_text"]))
                        for (_, row), prediction in zip(task_entries, task_predictions)
                    ],
                ),
            )
        )
        if on_task_complete is not None:
            maybe_awaitable = on_task_complete(predictions, task_records, task_id)
            if inspect.isawaitable(maybe_awaitable):
                await maybe_awaitable

    metrics = finalize_eval_metrics_from_task_records(task_records)
    return eval_examples, prediction_artifact_rows(predictions), metrics
...
async def run_train(
    ...
    """Run epoch-based mini-batch SFT training loop with LR scheduling, periodic eval, and periodic checkpoint.

    When ``--eval-every`` triggers after an ``optim_step``, eval scalars are merged into the same step's row in
    ``train/metrics.jsonl`` under ``test/`` keys (same idea as ``tinker_cookbook.supervised`` merging eval into
    ``log_metrics``). ``eval/periodic.jsonl`` still receives one line per eval for a stable eval-only stream.

    Rows are **appended** to ``train/metrics.jsonl`` as training runs (cleared with other run-scoped JSONL on fresh
    starts). Cadence is controlled by ``--train-metrics-every``; stdout uses ``--train-print-every``.
    """
    ...
    resume_checkpoint = get_last_resumable_checkpoint(log_path)
    ...
    train_metrics_path = log_path / "train" / "metrics.jsonl"
    train_batches_path = log_path / "train" / "batches.jsonl"
    ...
    periodic_eval_rows = eval_rows

    print(
        f"Training: {len(train_rows)} examples, {n_batches} batches x {num_epochs} epochs = {total_steps} steps, "
        f"batch_size={batch_size}, lr={base_lr}, schedule={lr_schedule}, rank={rank}, "
        f"periodic_eval_rows={len(periodic_eval_rows)}, "
        f"train_metrics_every={train_metrics_every}, train_print_every={train_print_every}, "
        f"batch_prompt_rows_per_step={DEFAULT_BATCH_PROMPT_RECORD_LIMIT}"
    )
    ...
    for epoch_idx in range(start_epoch_idx, num_epochs):
        ...
        for batch_idx in range(batch_start_idx, n_batches):
            ...
            learning_rate = base_lr * compute_lr_multiplier(lr_schedule, step, total_steps)

            batch_rows = shuffled[batch_idx * batch_size : (batch_idx + 1) * batch_size]
            ...
            fb_fn = getattr(training_client, "forward_backward_async", None)
            if callable(fb_fn):
                fwd_bwd_result = await resolve_api_result_async(fb_fn(datums, loss_fn="cross_entropy"))
            else:
                fwd_bwd_result = resolve_api_result(training_client.forward_backward(datums, loss_fn="cross_entropy"))

            final_loss, total_weight = compute_mean_nll(fwd_bwd_result, datums)
            if total_weight == 0:
                print("  warning: no valid loss weights in this batch")

            num_loss_tokens = int(total_weight)

            optim_fn = getattr(training_client, "optim_step_async", None)
            if callable(optim_fn):
                optim_step_result = await resolve_api_result_async(optim_fn(types.AdamParams(learning_rate=learning_rate)))
            else:
                optim_step_result = resolve_api_result(training_client.optim_step(types.AdamParams(learning_rate=learning_rate)))
            ...
            # Periodic checkpoint and eval AFTER optim_step (post-step weights)
            if save_every > 0 and step % save_every == 0:
                ...
                periodic_state = await save_training_state(training_client, state_save_name)
                if periodic_state:
                    checkpoint_row["state_path"] = periodic_state
                periodic_sampler_path = await save_sampler_checkpoint(training_client, sampler_save_name)
                if periodic_sampler_path:
                    checkpoint_row["sampler_path"] = periodic_sampler_path
                if "state_path" in checkpoint_row or "sampler_path" in checkpoint_row:
                    ...
                    details = [f"{key}={value}" for key, value in checkpoint_row.items() if key != "step"]
                    print(f"  Checkpoint at step {step}: {', '.join(details)}")
            ...
            if eval_every > 0 and step % eval_every == 0:
                ...
                sample_preds = [
                    ...
                    for p in periodic_predictions[:PERIODIC_EVAL_SAMPLE_COUNT]
                ]
                append_jsonl(log_path / "eval" / "periodic.jsonl", {"step": step, **scalar_metric_items(periodic_metrics), "samples": sample_preds})
                print(f"  Periodic eval at step {step}: eval_lawbench_avg={periodic_metrics.get('eval_lawbench_avg', 0):.4f}")
            ...
            eval_suffix = ""
            if step_eval_metrics is not None:
                eval_suffix = f" test/eval_lawbench_avg={float(step_eval_metrics.get('eval_lawbench_avg', 0)):.4f}"
            if should_print_train_step(step, total_steps, train_print_every):
                print(
                    ...
                    f"tok/s={tokens_per_sec:.0f} step_time={step_time:.1f}s{eval_suffix}"
                )
        if max_steps is not None and step >= max_steps:
            break
...
async def main_async() -> int:
    ...
    eval_rows = [
        {
            **row,
            "_dataset_name": eval_path.stem,
            "_source_eval_data": str(eval_path),
            "_source_index": source_index,
        }
        for source_index, row in enumerate(eval_rows, start=1)
    ]

    train_eval_path = eval_path
    train_eval_rows = eval_rows
    train_eval_arg = str(getattr(args, "train_eval_data", "") or "").strip()
    if train_eval_arg:
        train_eval_path = Path(train_eval_arg)
        if not train_eval_path.exists():
            raise RuntimeError(f"train eval data not found at {train_eval_path}")
        train_eval_rows = normalize_eval_rows(train_eval_path)
        if args.eval_limit > 0:
            train_eval_rows = train_eval_rows[: args.eval_limit]
        train_eval_rows = [
            {
                **row,
                "_dataset_name": train_eval_path.stem,
                "_source_eval_data": str(train_eval_path),
                "_source_index": source_index,
            }
            for source_index, row in enumerate(train_eval_rows, start=1)
        ]
    ...
    overlap = audit_overlap(
        ...
        [
            extract_row_id(r)
            for r in (eval_rows if train_eval_path == eval_path else (eval_rows + train_eval_rows))
        ],
    )
    ...
    try:
        ...
        # ---- Eval-only (sampling client only, no LoRA) ----
        if args.eval_only:
            sampler = await create_sampling_client(service_client, args.base_model)
            tokenizer = get_tokenizer(sampler, args.base_model)
            total_eval_tasks = len(group_eval_rows_by_task(eval_rows))
            progress_eval_examples = [
                eval_example_artifact_row(
                    row,
                    messages=build_eval_messages(row),
                    example_id=extract_row_id(row, fallback=f"example-{index}"),
            ...
            eval_examples, predictions, metrics = await run_eval(
                ...
                eval_rows,
                args,
                on_task_complete=lambda partial_predictions, partial_task_records, completed_task_id: persist_eval_progress(
                    run_dir,
                    args=args,
                    eval_examples=progress_eval_examples,
                    predictions=partial_predictions,
                    task_records=partial_task_records,
                    completed_task_id=completed_task_id,
                    total_task_count=total_eval_tasks,
            ...
            write_outputs(
                ...
                extra_payload={
                    "eval_data": str(eval_path),
                    "train_eval_data": str(train_eval_path),
                    "overlap": overlap,
                },
            )
        ...
        # ---- SFT training (per scaffolds/profiles/sft.md) ----
        if not train_rows:
        ...
        print(
            ...
            f"rank={args.rank}, vocab={tokenizer.vocab_size:,}"
        )
        ...
        if state_path:
            final_checkpoint["state_path"] = state_path
        if sampler_path:
            final_checkpoint["sampler_path"] = sampler_path
        if state_path or sampler_path:
            append_jsonl(run_dir / "train" / "checkpoints.jsonl", final_checkpoint)
