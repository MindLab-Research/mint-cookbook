...
LawBench has already promoted a concrete train-and-eval recipe into the wrapper. The current default recipe is:
...
- Primary search signal: `METRIC eval_lawbench_avg=...` from the wrapper run
...
- the role of `uv run train.py --eval-only` as the bare benchmark confirmation entrypoint
...
- rerunning the same training command in the same `--log-path` continues the same interrupted candidate run
- same-run resume comes from the latest resumable `state_path` in `--log-path/train/checkpoints.jsonl`
- `state_path` stays the resumable restore handle and `sampler_path` stays the later eval-only handle
- `--load-checkpoint-path` is for fresh-run weight loading only and should be used intentionally, not as the default search strategy
- for a clean confirmation rerun or later comparison, read the latest `sampler_path` from `train/checkpoints.jsonl` and rerun `uv run train.py --eval-only --eval-data data/eval/full.jsonl --base-model <sampler_path>`
