...
> For legacy scripts that already import `tinker`, the smallest bridge is
> `import mint as tinker`; then move endpoint and auth wiring onto
> `MINT_BASE_URL` and `MINT_API_KEY`. Use that alias only as a migration bridge,
> not as the default style for new code.
...
| `HF_HOME` | Local Hugging Face cache root | `~/.cache/huggingface` |
