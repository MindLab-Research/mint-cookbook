...
4. A person or agent should be able to enter one experiment directory and understand:
   ...
   - how to train it
   - how to evaluate it
   - how to confirm the benchmark path with `uv run train.py --eval-only --eval-data <full_eval_path>`
   - how to automate repeated benchmark runs through `pi-autoresearch`
...
- `.env.example` - the shared checked-in live env template with credential and endpoint placeholders
...
- `autoresearch.sh` - automation wrapper for `pi-autoresearch` around the current practical line
- `autoresearch.md` - wrapper protocol plus benchmark-scope notes for `pi-autoresearch`
...
- `uv run train.py --eval-only --eval-data <full_eval_path>` is the canonical benchmark entrypoint; `autoresearch.sh` is the canonical automation wrapper around that path.
- Baseline scaffold outputs stay minimal but explicit: `run.json`, `console.log`, `eval/examples.jsonl`, `eval/predictions.jsonl`, and `eval/metrics.json`, unless the experiment explicitly graduates to research-grade logging.
...
2. Read that experiment's `AGENTS.md` and `PROMPT.md` when they exist, then `README.md` and `train.py`.
...
- in `retrofit` mode, keep existing scaffold files in place, validate that the required canonical scaffold set is present after the run, and backfill only missing canonical scaffold files without inventing a second template family
- let root harness files still obey `--overwrite`; an explicit `--mode retrofit --overwrite` refresh may rewrite root `README.md`, `AGENTS.md`, `.env.example`, `.gitignore`, and `experiments/README.md` while still preserving existing scaffold files
