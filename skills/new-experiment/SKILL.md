...
description: |
  ...
  and add basic SFT, DPO, or GRPO training by following `scaffolds/profiles/`.
...
3. **Basic training** — follow `scaffolds/profiles/sft.md`, `scaffolds/profiles/dpo.md`, or `scaffolds/profiles/grpo.md` to add SFT, DPO, or GRPO
...
- User wants to add basic SFT, DPO, or GRPO training to an existing eval baseline
...
- `scaffolds/single_file_experiment/env.tpl` (source template for a local `.env` seed only; the repo-root `.env.example` stays the only checked-in live env template)
...
- The default scaffold README flow is `Quickstart`, `Fast contract tests`, `Live smoke tests`, `Data`, `Current results`, `References`; start `Current results` with `Status: \`placeholder\`` until a checked run exists, then switch it to `Status: \`checked\`` only when the reported run is actually checked.
...
|-- README.md
|-- train.py
|-- .env                                  # local, gitignored; copied from the repo-root `.env.example` or materialized from scaffold guidance
|-- autoresearch.sh
|-- autoresearch.md
`-- data/
    |-- eval/
...
8. Keep data paths CLI-driven through `--train-data` and `--eval-data`. When the experiment keeps the standard split layout, pass `data/eval/smoke.jsonl` explicitly for `--dry-run` validation and `data/eval/full.jsonl` explicitly for the benchmark path instead of auto-filling them in code. Multi-dataset experiments may instead use documented dataset-family eval specs.
9. Prefer the repo-wide data workflow for new experiments: use `data/eval/` plus `data/train/` for single-dataset layouts, or a documented dataset-family layout under `data/` when the experiment really spans multiple upstream datasets or benchmark suites. Preserve raw snapshots when local materialization is needed, keep download or snapshot scripts plus build or adjustment scripts under `data/`, and materialize smoke before full when a cheap slice exists.
...
17. Do not add SFT, DPO, or GRPO code until the eval contract, prompt shape, parser, grader, and metric are stable.
...
This repo is a MinT cookbook, and new experiments should start directly on the maintained `mint` runtime surface. The standard development sequence is: get the experiment working on mint first (dry-run → eval-only → training), then keep that mint-facing runtime contract stable while growing the experiment.
...
Keep backend-specific code inside one clearly labeled backend compatibility section so migration stays localized, as described in `scaffolds/single_file_experiment/naming.md` and the canonical `train.py.tpl`.
...
- Only then follow `scaffolds/profiles/sft.md`, `scaffolds/profiles/dpo.md`, or `scaffolds/profiles/grpo.md` to add training.

## pi-autoresearch expectations

- `autoresearch.sh` must stay a thin wrapper around the project command, usually `uv run train.py`.
- Use the current wrapper shape: resolve `SCRIPT_DIR`, `cd` into the experiment, define a named `LOG_PATH` under `artifacts/runs/`, then `exec uv run train.py ... "$@" 2>&1`.
...
- run `cd experiments/<name> && uv sync && uv run train.py --dry-run --eval-data <smoke_eval_path>`
- run `cd experiments/<name> && uv run train.py --eval-only --eval-data <full_eval_path>` to establish the frozen runnable benchmark baseline
- to add basic training, follow `scaffolds/profiles/sft.md`, `scaffolds/profiles/dpo.md`, or `scaffolds/profiles/grpo.md`
