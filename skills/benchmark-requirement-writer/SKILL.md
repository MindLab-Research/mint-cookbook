...
- use `uv run train.py --eval-only --eval-data <full_eval_path>` as the benchmark entrypoint
- use `uv run train.py` as the train entrypoint
- use `uv run train.py --dry-run --eval-data <smoke_eval_path>` as the dry-run validation entrypoint when possible
