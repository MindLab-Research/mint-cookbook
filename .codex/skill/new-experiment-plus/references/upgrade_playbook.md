...
4. Add checkpoint restore support with split semantics: same-run `--resume-from` versus fresh-run `--load-checkpoint-path`, with `train/checkpoints.jsonl` as the restore source of truth.
...
Basic LR scheduling (`--lr-schedule`), periodic eval (`--eval-every`), and periodic checkpoint (`--save-every`) already belong to `scaffolds/profiles/sft.md`. This playbook starts after that layer: it adds per-step timing and throughput logging, non-blocking background eval, checkpoint-aware sampler snapshots, and richer checkpoint registries.
...
Keep the baseline README flow (`Quickstart`, `Data`, `Current results`, `Outputs`, `References`) unless the experiment has a strong reason to diverge. Measured runs should keep the evidence bundle together: train config when relevant, eval config, result metrics, and wall-clock timing. Eval-only baselines still count as measured runs.

Where to edit together:

- `scaffolds/README.md` -> `Contract split: who owns what`
- `.codex/skill/new-experiment-plus/SKILL.md` -> `Research-grade operability`
...
Append-only registries should still be run-scoped: truncate or rotate them when starting a fresh run or when using `--load-checkpoint-path`; preserve them only when `--resume-from` is deliberately continuing the same registry in the same run directory.
...
Stay with `scaffolds/profiles/sft.md` or `scaffolds/profiles/grpo.md` if the user only wants basic training added on top of the baseline and does not need extra operability layers.
