...
- checkpoint restore flow with split semantics when long runs matter: same-run `--resume-from` versus fresh-run `--load-checkpoint-path`
...
Before treating a plus upgrade as done, walk `.codex/skill/new-experiment-plus/SKILL.md` -> `Research-grade operability` and confirm:
...
- If a feature changes how a run survives or restores from checkpoints, put it in `train.py` and usually in a machine-readable output or registry, and keep the same-run `--resume-from` path separate from fresh-run `--load-checkpoint-path`.
